...
func (grpcClient *DefaultGRPCClient) FetchGuildByID(grpcContext *GRPCContext, guildID discord.Snowflake) (guild discord.Guild, err error) {
	...
	if err != nil {
		return guild, fmt.Errorf("failed to fetch guilds: %w", err)
	}
	...
	if grpcGuild != nil {
		...
		if err != nil {
			return guild, fmt.Errorf("failed to convert protobuf.Guild to Guild: %w", err)
		}
	}
...
func (grpcClient *DefaultGRPCClient) FetchChannelByID(grpcContext *GRPCContext, guildID discord.Snowflake, channelID discord.Snowflake) (channel discord.Channel, err error) {
	...
	if err != nil {
		return channel, fmt.Errorf("failed to fetch channels: %w", err)
	}
	...
	if grpcChannel != nil {
		...
		if err != nil {
			return channel, fmt.Errorf("failed to convert protobuf.Channel to Channel: %w", err)
		}
	}
...
func (grpcClient *DefaultGRPCClient) FetchRoleByID(grpcContext *GRPCContext, guildID discord.Snowflake, roleID discord.Snowflake) (role discord.Role, err error) {
	...
	if err != nil {
		return role, fmt.Errorf("failed to fetch roles: %w", err)
	}
	...
	if grpcRole != nil {
		...
		if err != nil {
			return role, fmt.Errorf("failed to convert protobuf.Role to Role: %w", err)
		}
	}
...
func (grpcClient *DefaultGRPCClient) FetchEmojiByID(grpcContext *GRPCContext, guildID discord.Snowflake, emojiID discord.Snowflake) (emoji discord.Emoji, err error) {
	...
	if err != nil {
		return emoji, fmt.Errorf("failed to fetch emojis: %w", err)
	}
	...
	if grpcEmoji != nil {
		...
		if err != nil {
			return emoji, fmt.Errorf("failed to convert protobuf.Emoji to Emoji: %w", err)
		}
	}
...
func (grpcClient *DefaultGRPCClient) FetchMemberByID(grpcContext *GRPCContext, guildID discord.Snowflake, memberID discord.Snowflake) (member discord.GuildMember, err error) {
	...
	if err != nil {
		return member, fmt.Errorf("failed to fetch members: %w", err)
	}
	...
	if grpcMember != nil {
		...
		if err != nil {
			return member, fmt.Errorf("failed to convert protobuf.GuildMember to GuildMember: %w", err)
		}
	}
...
func (grpcClient *DefaultGRPCClient) FetchUserByID(grpcContext *GRPCContext, token string, userID discord.Snowflake, createDMChannel bool) (user discord.User, err error) {
	...
	if err != nil {
		return user, fmt.Errorf("failed to fetch users: %w", err)
	}
	...
	if grpcUser != nil {
		...
		if err != nil {
			return user, fmt.Errorf("failed to convert protobuf.User to User: %w", err)
		}
	}
...
func (grpcClient *DefaultGRPCClient) FetchConsumerConfiguration(grpcContext *GRPCContext, identifier string) (identifiers sandwich_structs.SandwichConsumerConfiguration, err error) {
	...
	if err != nil {
		return identifiers, fmt.Errorf("failed to fetch consumer configuration: %w", err)
	}
	...
	if err != nil {
		return identifiers, fmt.Errorf("failed to unmarshal consumer configuration: %w", err)
	}
